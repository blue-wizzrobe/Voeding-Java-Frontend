...
function toevoegenAanTabel(recepten) {
    ...
    for(var x=0; x<recepten.length; x++) {
        data.innerHTML+=`<div class="row">
            <div class="col-sm-3" style="font-weight: 600;">${recepten[x].bereidingstijd} minuten</div>
            <div class="col-sm-3" style="font-weight: 600;  text-decoration: underline;" onclick="naarReceptenDetails(${recepten[x].id})">${recepten[x].naam}</div>
            <div class="col-sm-3" style="font-weight: 600;">${recepten[x].beschrijving}</div>
            <div class="col-sm-3" style="font-weight: 600;"><img src="afb/${recepten[x].afbeelding}" /></div>
        </div>`;
    }
}
